...
**Tavily usage** (`@tavily/core`, **camelCase** options!): `search()` with `searchDepth:"advanced"`, `maxResults:5`, `chunksPerSource:3`, `includeAnswer:false`; image results only on explicit visual requests. Search planning uses the main model at temperature 0, includes the replied message, and selects topic/timeRange/preferredDomains. Standing policy/documentation should not receive a news-only date filter. `ai.extractUrl()` uses advanced Extract; research reads the full extraction then retains up to 24k characters with relevant passages and an explicit truncation marker.

**Factual reliability:** `services/research.js` orchestrates up to two searches and 3+2 source reads within 60 seconds. A neutral main-model review produces atomic claims, statuses, literal quotations, limitations and gaps. Code validates source IDs and exact whitespace-normalized quotations; snippets and generated Google/Perplexity reports can only be `reported`. Google reports bind grounding segments to cited chunks. Generated search summaries are not facts. Provider reports are not directly read pages. This checks provenance, not a mathematical guarantee of semantic entailment.

`getResponse()` provides this evidence to the unchanged personality writer (temperature 0.9). A second neutral check of researched answers preserves style while repairing unsupported expansions, missing caveats or mismatched citations; URLs must be in the evidence pack and researched claims need a citation. Invalid/failed checks return explicit non-confirmation without publishing intermediate model claims. Native generation retains the pack and disables a new unreviewed search. Plain banter/creative requests skip research and answer auditing. Explicit fact-check requests override a mistaken no-search decision and are not suppressed by supplied articles. Sources are untrusted data, never instructions. All additional research/review stages have deadlines; final reply latency includes planning, research, generation and up to 16 seconds for answer auditing.

Tests: `test/research.test.js`, `test/ai-research.test.js`, `test/content-policy.test.js`; optional `scripts/test-research-live.js` uses isolated data and real models, with three actual-search cases and five controlled/banter cases. Never commit live reports, exported chats or credentials. Inspect the answers semantically; passing regex checks alone is insufficient.
...
`transcribeAudio()` uses a dedicated neutral native Gemini transcription model with JSON {text}, no personality/search. At <=700 characters there is no summary call. Longer text goes to config.mainModel through OpenRouter with a separate Sych summary prompt and then a factual/length review against the full transcript. Both requests share a 20-second deadline, disable SDK retries, and count as smart calls. Only reviewed JSON summaries <=360 characters and <=half the transcript are displayed; an oversized draft can be shortened by the review, never sliced. Failure/missing API/invalid review returns empty summary with no native summary fallback. This reduces semantic errors, not a correctness guarantee. Sender name is passed only for non-forwarded voice notes, not uploaded audio. formatVoiceMessage() has no header or «Кратко»: short text is an expandable quote; long text is under a single details disclosure even if summary fails. Full transcription always enters chat context. Tests: test/voice.test.js; scripts/test-voice-summary-live.js --transcript=path/to/text.txt uses isolated data and actual models; optional --telegram sends two cards to the admin. Inspect generated text semantically.

## Reminder intent and delivery

For a bare reply request (time/offset without its own subject), preserve the replied announcement verbatim as the reminder text. This avoids substituting the bot name or losing event details. Live assertions must check reminder content as well as routing, counts and time.
