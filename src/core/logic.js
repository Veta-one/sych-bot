...
function getSychErrorReply(errText) {
    ...
    const phrases = [
        "🛠 Так, у меня шестеренки встали. Какая-то дичь в коде. @VETAone, просыпайся, тут всё сломалось!",
        "💥 Я упал. Критическая ошибка. @VETAone, чини давай, я работать не могу.",
        "🚑 Хьюстон, у нас проблемы. Я поймал баг и не знаю, что делать. @VETAone, выручай."
    ];
    return phrases[Math.floor(Math.random() * phrases.length)];
}
  ...
  if (!storage.hasChat(chatId) && chatId !== config.adminId) {
    ...
    if (msg.chat.type === 'private' && userId !== config.adminId) {
        ...
        if (command !== '/start') {
            ...
            const infoText = `👋 **Йо! В личке я общаюсь только с Админом.**
